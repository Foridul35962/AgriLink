import { CATEGORY_VALUES } from "@/constants/constantValues";
import type { AppTranslations } from "../types";

const en: AppTranslations = {
  nav: {
    crops: "Crops",
    cropSuggestions: "Crop Suggestions",
    product: "Products",
    inventory: "Inventory",
    login: "Log in",
    getStarted: "Get started",
    dashboard: "Dashboard",
  },
  logout: {
    title: "Do you want to log out?",
    message: "You will be signed out of your account. You’ll need to log in again to continue.",
    cancel: "Cancel",
    confirm: "Log out",
  },
  languageSwitcher: {
    label: "Language",
    english: "English",
    bangla: "বাংলা",
  },
  hero: {
    badge: "Connecting every link of the agri chain",
    titleLine: "From the field to the market,",
    titleHighlight: "grown together.",
    subtitle:
      "AgriLink connects farmers, aratdars and retailers on one platform — fair prices, fewer middlemen, faster deliveries.",
    ctaPrimary: "Create free account",
    ctaSecondary: "See how it works",
    roles: {
      farmer: "Farmers",
      aratdar: "Aratdars",
      retailer: "Retailers",
    },
  },
  stats: {
    items: [
      { value: "12,000+", label: "Registered farmers" },
      { value: "64", label: "Districts covered" },
      { value: "50,000+", label: "Tons traded" },
      { value: "8,500+", label: "Direct connections made" },
    ],
  },
  features: {
    sectionLabel: "Why AgriLink",
    title: "Everything the agri chain needs, in one place",
    subtitle:
      "Built for the realities of farming and trade in Bangladesh — simple, transparent, and fair for everyone involved.",
    items: [
      {
        title: "Real-time market prices",
        description:
          "See up-to-date crop prices across every district before you buy or sell.",
      },
      {
        title: "Direct trade, less waste",
        description:
          "Connect farmers straight to buyers — fewer middlemen, fresher produce, better margins.",
      },
      {
        title: "Verified accounts",
        description:
          "Every farmer, aratdar, and retailer is verified so you always know who you're trading with.",
      },
      {
        title: "Secure payments",
        description:
          "Pay and get paid safely with transaction protection built into every order.",
      },
      {
        title: "Direct messaging",
        description:
          "Negotiate quantities, prices, and delivery details directly in the app.",
      },
      {
        title: "Order tracking",
        description:
          "Track every order from harvest to delivery, with status updates at each step.",
      },
    ],
  },
  howItWorks: {
    sectionLabel: "How it works",
    title: "Four steps from harvest to hand",
    stepLabel: "STEP",
    steps: [
      {
        title: "Create your account",
        description:
          "Sign up in minutes and tell us whether you're a farmer, aratdar or retailer.",
      },
      {
        title: "List or browse",
        description:
          "Farmers list their harvest, buyers browse listings by crop, price, and district.",
      },
      {
        title: "Connect & agree",
        description:
          "Message directly, agree on quantity and price, and confirm the order.",
      },
      {
        title: "Deliver & get paid",
        description:
          "Track delivery in real time and receive secure payment once it's complete.",
      },
    ],
  },
  roles: {
    sectionLabel: "Who it's for",
    title: "Built for every link in the chain",
    items: [
      {
        title: "Farmers",
        description: "Sell your harvest directly and get fair, transparent prices.",
        points: ["List crops in seconds", "See live market prices", "Get paid securely"],
      },
      {
        title: "Aratdars",
        description: "Source produce reliably and manage bulk trade with ease.",
        points: [
          "Bulk order management",
          "Verified farmer network",
          "Faster settlements",
        ],
      },
      {
        title: "Retailers",
        description: "Stock fresh produce straight from source, at better margins.",
        points: ["Direct sourcing", "Consistent supply", "Order tracking"],
      },
    ],
  },
  testimonials: {
    sectionLabel: "Trusted across the chain",
    title: "What our community is saying",
    items: [
      {
        quote:
          "AgriLink helped me sell my rice harvest directly to an aratdar in Dhaka. No middleman, and I got a much better price than before.",
        name: "Abdul Karim",
        role: "Farmer, Bogura",
      },
      {
        quote:
          "Sourcing vegetables used to take days of phone calls. Now I browse listings and confirm orders in the same afternoon.",
        name: "Salma Begum",
        role: "Retailer, Chattogram",
      },
      {
        quote:
          "As an aratdar, tracking who I'm buying from and settling payments used to be messy. This platform made it simple.",
        name: "Mizanur Rahman",
        role: "Aratdar, Rangpur",
      },
    ],
  },
  cta: {
    title: "Ready to grow with AgriLink?",
    subtitle:
      "Join thousands of farmers, aratdars and retailers already trading smarter, together.",
    button: "Create your free account",
  },
  footer: {
    description:
      "Connecting farmers, aratdars and retailers across Bangladesh — fair prices, fewer middlemen.",
    email: "support@agrilink.com",
    phone: "+880 1XXX-XXXXXX",
    sections: {
      platform: {
        title: "Platform",
        links: [
          { label: "Features", href: "#features" },
          { label: "How it works", href: "#how-it-works" },
          { label: "Who it's for", href: "#roles" },
        ],
      },
      company: {
        title: "Company",
        links: [
          { label: "About us", href: "#" },
          { label: "Contact", href: "#contact" },
          { label: "Careers", href: "#" },
        ],
      },
      legal: {
        title: "Legal",
        links: [
          { label: "Privacy policy", href: "#" },
          { label: "Terms of service", href: "#" },
        ],
      },
    },
    copyright: "All rights reserved.",
  },
  addProduct: {
    badge: "List a new product",
    title: "Add your product",
    subtitle:
      "Fill in the details below so buyers can find and order your product.",
    fields: {
      name: { label: "Product name", placeholder: "e.g. Premium Basmati Rice" },
      category: { label: "Category", placeholder: "Select category" },
      quantity: { label: "Quantity", placeholder: "e.g. 500" },
      unit: { label: "Unit" },
      pricePerUnit: { label: "Price per unit (৳)", placeholder: "e.g. 45.50" },
      district: { label: "District", placeholder: "Select district" },
      harvestDate: { label: "Harvest date" },
      description: {
        label: "Description (optional)",
        placeholder: "Tell buyers more about quality, variety, storage, etc.",
        charsRemaining: "characters remaining",
      },
      image: {
        label: "Product image",
        dragText: "Drag & drop an image here, or",
        browseText: "browse",
        requirements: "PNG or JPG, up to 5MB",
        change: "Change image",
        remove: "Remove",
      },
    },
    categories: CATEGORY_VALUES,
    units: { kg: "Kilogram (kg)", mon: "Mon", ton: "Ton", piece: "Piece" },
    errors: {
      nameRequired: "Product name is required",
      categoryRequired: "Category is required",
      quantityRequired: "Quantity is required",
      quantityMin: "Quantity must be greater than 0",
      priceRequired: "Price is required",
      priceMin: "Price must be greater than 0",
      districtRequired: "District is required",
      districtInvalid: "Invalid district",
      harvestDateRequired: "Harvest date is required",
      harvestDateInvalid: "Harvest date must be a valid date",
      unitRequired: "Unit is required",
      descriptionMax: "Description must be under 300 characters",
      imageRequired: "Product image is required",
    },
    submitButton: "Add product",
    submittingButton: "Adding product...",
    serverError: "Something went wrong. Please try again.",
  },
  adminMemberRequests: {
    header: {
      title: "Member Approval Requests",
      subtitle: "Review and approve new registered member requests",
    },
    filter: {
      allRoles: "All Roles",
      farmer: "Farmer (কৃষক)",
      aratdar: "Aratdar (আড়তদার)",
      retailer: "Retailer (খুচরা বিক্রেতা)",
    },
    stats: {
      showing: "Showing",
      of: "of",
      pendingRequests: "pending requests",
      filterLabel: "Filter",
    },
    emptyState: {
      title: "No Pending Requests Found",
      subtitle: "There are currently no member requests waiting for approval in this category.",
    },
    table: {
      memberName: "Member Name",
      role: "Role",
      contactInfo: "Contact Info",
      district: "District",
      actions: "Actions",
    },
    roles: {
      farmer: "farmer",
      aratdar: "aratdar",
      retailer: "retailer",
    },
    actions: {
      accept: "Accept",
      reject: "Reject",
    },
    pagination: {
      page: "Page",
      of: "of",
    },
  },
  myProducts: {
    title: "My Products",
    subtitle: "Manage your added products, stock status, and listings.",
    allCategories: "All Categories",
    allStatuses: "All Status",
    showing: "Showing",
    of: "of",
    products: "products",
    noProductsFound: "No Products Found",
    noProductsDesc: "There are no products matching your selected filters.",
    quantity: "Quantity",
    pricePer: "Price /",
    totalValue: "Total Value:",
    page: "Page",
    categories: {
      Rice: "Rice",
      Wheat: "Wheat",
      Vegetables: "Vegetables",
      Fruits: "Fruits",
      Spices: "Spices",
      Pulses: "Pulses",
      Jute: "Jute",
      Fish: "Fish",
      "Poultry & Dairy": "Poultry & Dairy",
      Other: "Other",
    },
    status: {
      available: "Available",
      sold: "Sold Out",
      expired: "Expired",
    },
  },
  productDetail: {
    loading: "Loading product details...",
    edit: "Edit",
    delete: "Delete",
    bidsLocked: "Editing/Deletion locked (Bids placed)",
    quantity: "Quantity",
    pricePerUnit: "Price Per Unit",
    harvestDate: "Harvest Date",
    description: "Description",
    farmerInfo: "Farmer Info",
    liveAuction: "Live Auction",
    startPrice: "Start Price",
    highestBid: "Highest Bid",
    topBids: "Top Bids",
    bidsCount: "Bids",
    noBids: "No bids placed yet.",
    winner: "Auction Winner",
    deleteModalTitle: "Confirm Deletion",
    deleteModalDesc: "Are you sure you want to delete this product? This action cannot be undone.",
    cancel: "No, Keep it",
    confirmDelete: "Yes, Delete",
    deleting: "Deleting...",
    placeBid: "Place Bid",
    enterBidAmount: "Enter Bid Amount",
    bidAmountPlaceholder: "Enter amount (৳)",
    submitBid: "Submit Bid",
    submitting: "Submitting...",
    selectWinner: "Select Winner",
    selecting: "Selecting...",
    bidModalTitle: "Place Your Bid",
    auctionEnded: "Auction Ended",
    auctionEndTime: "Auction End Time:",
    locale: "en-US",
    orderLoading: "Creating Order...",
    orderNow: "Order Now"
  },
  editProduct: {
    badge: "Edit Product",
    title: "Edit Product Details",
    subtitle: "Update product information and save changes.",
    noChanges: "No changes were made.",
    updateSuccess: "Product updated successfully!",
    updateError: "Failed to update product",
    updating: "Updating...",
    saveChanges: "Save Changes",
    fetchError: "Failed to fetch product details",
  },
  adminReports: {
    badge: "Admin Portal",
    title: "Submitted Reports",
    subtitle: "Manage and review all system reports submitted by users.",
    table: {
      id: "Report ID",
      topic: "Topic / Summary",
      action: "Action",
      viewDetails: "View Details",
      emptyState: "No reports found.",
    },
    emptyState: {
      title: "No Reports Found",
      description:
        "There are currently no reports submitted in the system. Check back later or refresh the list.",
      refresh: "Refresh List",
    },
    pagination: {
      showing: "Showing",
      of: "of",
      reports: "reports",
      previous: "Previous",
      next: "Next",
      page: "Page",
    },
  },
  createReport: {
    badge: "Report System",
    title: "Submit a Report",
    subtitle: "Please provide details about the issue or violation you encountered.",
    fields: {
      topic: {
        label: "Report Topic / Reason",
        placeholder: "e.g., Inappropriate behavior, Fraudulent activity, Spam",
      },
      description: {
        label: "Detailed Description",
        placeholder: "Explain what happened in detail...",
        charsRemaining: "characters left",
      },
      reportedUser: {
        label: "Reported Target User ID",
      },
    },
    buttons: {
      submit: "Submit Report",
      submitting: "Submitting...",
      cancel: "Cancel",
    },
    messages: {
      success: "Report submitted successfully.",
      error: "Failed to submit report. Please try again.",
      topicRequired: "Topic is required",
      topicLength: "Topic must be between 10 and 50 characters",
      descriptionRequired: "Description is required",
      descriptionLength: "Description must be between 50 and 300 characters",
    },
  },
  adminReport: {
    backToDashboard: "Back to Dashboard",
    reviewed: "Reviewed",
    pendingReview: "Pending Review",
    reportId: "Report ID",
    description: "Description",
    submittedOn: "Submitted on",
    reportedTarget: "Reported Target",
    reporter: "Reporter",
    removeReportedUser: "Remove Reported User",
    removeReporter: "Remove Reporter",
    adminReviewActions: "Admin Review Actions",
    adminActionsDesc: "Send warning to the reported user or mark this report as reviewed.",
    sendWarning: "Send Warning",
    markAsDone: "Mark as Done",
    alreadyReviewed: "Already Reviewed",
    reportedUserHistory: "Reported User Offense History",
    reporterHistory: "Reporter Submission History",
    noOffenses: "No previous offenses found.",
    noSubmissions: "No previous submissions found.",
    resolved: "Resolved",
    pending: "Pending",
    sendWarningTitle: "Send Warning Action",
    sendWarningDesc: "Are you sure you want to send an official warning to",
    sendWarningSuffix: "regarding this report?",
    cancel: "Cancel",
    confirmAndSend: "Confirm & Send",
    removeUserTitle: "Remove User Account",
    removeUserDesc: "You are about to remove",
    removeUserSuffix: ". This action cannot be undone.",
    reasonLabel: "Reason for Removal",
    reasonPlaceholder: "Provide a clear reason for removing this account...",
    confirmRemoval: "Confirm Removal",
    unknownUser: "Unknown User",
    unknownReporter: "Unknown Reporter",
    reporterRole: "Reporter",
    reportedUserRole: "Reported User",
    loadingDetails: "Loading Report Details...",
  },
  productsPage: {
    title: "Fresh Agricultural Market",
    subtitle: "Directly connecting local farmers with consumers for pure, authentic, and fresh produce across Bangladesh.",
    searchPlaceholder: "Search for crops, fruits, or items...",
    categoryLabel: "Category",
    districtLabel: "District",
    allCategories: "All Categories",
    allDistricts: "All Districts",
    resetFilters: "Clear Filters",
    noProducts: "We couldn't find any products matching your search criteria.",
    pricePerUnit: "Price",
    availableQty: "In Stock",
    previous: "Prev",
    next: "Next",
    pageLabel: "Page",
  },
  aratdarOrders: {
    title: "Your Placed Orders",
    subtitle: "Track and manage all orders you have placed",
    orderId: "Order ID",
    product: "Product",
    quantity: "Quantity",
    totalAmount: "Total Amount",
    status: "Status",
    date: "Date",
    noOrders: "No Orders Found",
    noOrdersSub: "You haven't placed any orders yet.",
    viewDetails: "View Details",
    currency: "৳",
    page: "Page",
    of: "of",
    previous: "Previous",
    next: "Next",
  },
  aratdarOrderDetails: {
    back: "Back",
    title: "Order Receipt",
    orderInfo: "Order Info",
    quantity: "Quantity",
    totalAmount: "Total Amount",
    currency: "৳",
    sellerInfo: "Seller Info",
    sellerName: "Seller Name",
    phoneNumber: "Phone Number",
    email: "Email",
    notProvided: "Not provided",
    notFound: "Order Not Found",
    notFoundSub: "We couldn't find details for this order, or it may have been removed.",
    report: "Report"
  },
  farmerReceiveOrders: {
    title: "Received Orders",
    subtitle: "View all incoming buyer orders for your produce and track their statuses",
    ledgerEntries: "Total Orders",
    placed: "Placed",
    totalAmount: "Total Amount",
    currency: "৳",
    noOrders: "No Orders Received",
    noOrdersSub: "You haven't received any orders yet. Incoming buyer orders will appear here.",
    page: "Page",
    of: "of",
    previous: "Previous",
    next: "Next",
  },
  farmerReceiveOrderDetails: {
    back: "Back",
    title: "Received Order Receipt",
    orderInfo: "Order Info",
    quantity: "Quantity",
    totalAmount: "Total Amount",
    currency: "৳",
    buyerInfo: "Buyer Info",
    buyerName: "Buyer Name",
    phoneNumber: "Phone Number",
    email: "Email",
    district: "District",
    updateStatus: "Update Order Status",
    statusUpdatedSuccess: "Status updated successfully",
    updatingStatus: "Updating...",
    notProvided: "Not provided",
    notFound: "Order Not Found",
    notFoundSub: "We couldn't find details for this order, or it may have been removed.",
    statusOptions: {
      PROCESSING: "Processing",
      SHIPPED: "Shipped",
      DELIVERED: "Delivered",
    },
    report: "Report"
  },
  addCropPage: {
    eyebrow: "Crop Registry",
    pageTitle: "Add a new crop to the registry",
    step1Title: "Basic Crop Info",
    step1Sub: "Name, category, image and weather profile",
    step2Title: "Recommendations (Optional)",
    step2Sub: "Optional — districts, season and tips",
    addCropHeading: "Add New Crop",
    addRecHeading: "Where and when should this crop be grown?",
    identitySection: "Identity",
    cropNameEn: "Crop Name (English)",
    cropNameBn: "Crop Name (Bangla)",
    category: "Category",
    selectCategory: "Select Category",
    cropImage: "Crop Image",
    chooseImage: "Choose Image",
    changeImage: "Change Image",
    description: "Description",
    descriptionPlaceholder: "Provide a brief description of the crop...",
    cultivationSection: "Cultivation Details",
    waterRequirement: "Water Requirement",
    suitableSoil: "Suitable Soil",
    cultivationDuration: "Cultivation Duration",
    cultivationTips: "Cultivation Tips",
    cultivationTipsPlaceholder: "Provide guidelines or tips for cultivation...",
    weatherRequirement: "Weather Requirements",
    minTemp: "Min Temperature (°C)",
    maxTemp: "Max Temperature (°C)",
    maxHumidity: "Max Humidity (%)",
    maxRainProb: "Max Rain Probability (%)",
    saveAndContinue: "Save & Continue",
    saving: "Saving...",
    skipStep: "Skip this step",
    suitableDistricts: "Suitable Districts",
    plantingMonths: "Planting Months",
    season: "Season",
    selectSeason: "Select Season",
    reason: "Recommendation Reason",
    reasonPlaceholder: "Why is this crop suitable for selected regions/months?",
    tips: "Tips",
    addTip: "Add Another Tip",
    recNote: "These recommendations help farmers know exactly where and when to plant. You can always edit them later.",
    skip: "Skip",
    submitRec: "Submit Recommendation",
    updateRec: "Update Recommendation",
    update: "Update recommendation",
    submitting: "Submitting...",
    validation: {
      nameRequired: "Crop name is required",
      banglaNameRequired: "Bangla crop name is required",
      categoryRequired: "Category is required",
      imageRequired: "Crop image is required",
      minTempRequired: "Minimum temperature is required",
      maxTempRequired: "Maximum temperature is required",
      districtRequired: "At least one district is required",
      monthRequired: "At least one planting month is required",
    }
  },
  cropDetailsPage: {
    loading: "Loading crop details...",
    notFoundTitle: "Crop Not Found",
    notFoundDesc: "This crop may have been removed, or the link you followed is incorrect.",
    backButton: "Back to Crop List",
    editCrop: "Edit Crop",
    addRecommendation: "Add Recommendation",
    editRecommendation: "Edit Recommendation",
    category: "Category",
    description: "Description",
    waterRequirement: "Water Requirement",
    water: {
      low: "Low",
      medium: "Medium",
      high: "High",
    },
    cultivationDuration: "Cultivation Duration",
    days: "days",
    suitableSoil: "Suitable Soil",
    cultivationTips: "Cultivation Tips",
    weatherRequirement: "Weather Requirement",
    temperature: "Temperature",
    humidity: "Humidity",
    rainProbability: "Rain Probability",
    rainfall: "Rainfall",
    recommendationTitle: "Recommendation",
    noRecommendationTitle: "No Recommendation Yet",
    noRecommendationDesc: "No recommendation has been created for this crop yet.",
    districts: "Districts",
    plantingMonths: "Planting Months",
    season: "Season",
    reason: "Reason",
    tips: "Tips",
    seasons: {
      "kharif-1": "Kharif-1",
      "kharif-2": "Kharif-2",
      rabi: "Rabi",
      all: "All Seasons",
    },
    categories: {
      cereal: "Cereal",
      vegetable: "Vegetable",
      fruit: "Fruit",
      pulse: "Pulse",
      oilseed: "Oilseed",
      spice: "Spice",
      "cash-crop": "Cash Crop",
      other: "Other",
    },
  },
  cropList: {
    title: "Crop Directory",
    subtitle: "Explore crops suitable for your land and farming needs",

    loading: "Loading crops...",
    noCrops: "No crops found",
    noCropsDescription:
      "Try changing your search or filter to find more crops.",

    category: "Category",
    waterRequirement: "Water Requirement",
    suitableSoil: "Suitable Soil",

    page: "Page",
    of: "of",
    prev: "Previous",
    next: "Next",

    searchPlaceholder: "Search by crop name...",
    allCategories: "All Categories",
    searchBtn: "Search",
    clearFilters: "Clear Filters",

    showingResults: "Showing results",
    cropsFound: "crops found",

    categories: {
      cereal: "Cereal",
      vegetable: "Vegetable",
      fruit: "Fruit",
      pulse: "Pulse",
    },

    waterLevels: {
      low: "Low",
      medium: "Medium",
      high: "High",
    },
  },
  cropSuggestion: {
    title: "Crop Suggestion",
    selectDistrict: "Select District",
    selectDistrictLabel: "Select your district",
    loginPrompt: "Please select your district below to see crop suggestions",
    loading: "Loading...",
    noData: "No crop data found for this district",
    weatherTitle: "Weather",
    temperature: "Temperature",
    humidity: "Humidity",
    rainProbability: "Rain Probability",
    rainfall: "Rainfall",
    condition: "Condition",
    cropsFound: "crops found",
    waterRequirement: "Water Requirement",
    suitableSoil: "Suitable Soil",
  },
  inventory: {
    title: "Add New Crop Item",
    subtitle: "Add a new crop or agri-product to your inventory",
    basicInfoLabel: "Basic Information",
    quantityPricingLabel: "Quantity & Pricing",
    detailsLabel: "Details & Image",
    productNameLabel: "Crop / Product Name",
    productNamePlaceholder: "e.g. Miniket Rice, Wheat, Jute",
    categoryLabel: "Category",
    selectCategory: "Select Category",
    totalQuantityLabel: "Total Quantity",
    allocatedQuantityLabel: "Allocated Quantity",
    pricePerUnitLabel: "Price Per Unit",
    unitLabel: "Unit",
    selectUnit: "Select Unit",
    descriptionLabel: "Description (Optional)",
    descriptionPlaceholder: "Describe the crop — variety, quality, harvest time etc. (max 300 characters)",
    imageLabel: "Crop Image",
    imagePlaceholder: "Upload image",
    uploadClick: "Click to upload",
    uploadDrag: "or drag and drop",
    uploadHint: "PNG, JPG (max 5MB)",
    submitButton: "Save Inventory Item",
    submitting: "Saving...",
    units: {
      kg: "Kg",
      mon: "Mon",
      ton: "Ton",
      piece: "Piece",
    },
    errors: {
      productNameRequired: "Product name is required",
      categoryRequired: "Category is required",
      totalQuantityRequired: "Total quantity is required",
      totalQuantityMin: "Quantity must be at least 1",
      allocatedQuantityRequired: "Allocated quantity is required",
      allocatedQuantityMin: "Allocated quantity must be at least 1",
      pricePerUnitRequired: "Price per unit is required",
      pricePerUnitMin: "Price must be at least 1",
      unitRequired: "Unit is required",
      descriptionMax: "Maximum length of description is 300 characters",
      imageRequired: "Product image is required",
    },
  },
  inventoryDetails: {
    status: {
      available: "Available",
      depleted: "Out of Stock",
    },
    backButton: "Back to inventory",
    notFoundTitle: "Item not found",
    notFoundMessage: "This item may have been removed, or the link is incorrect.",
    totalQuantityCard: "Total Quantity",
    allocatedQuantityCard: "Allocated",
    availableQuantityCard: "Available",
    pricePerUnitCard: "Price per unit",
    descriptionTitle: "Description",
    noDescription: "No description has been added for this item.",
    orderButton: "Place Order",
    editButton: "Edit",
    deleteButton: "Delete",
    orderModal: {
      title: "Place an Order",
      outOfStockMessage: "Sorry, this item is currently out of stock.",
      quantityLabel: "Quantity",
      quantityRequiredError: "Please enter quantity",
      quantityMinError: "Please select at least 1 item",
      quantityMaxError: "Quantity cannot exceed available stock",
      submitButton: "Confirm Order",
      closeButton: "Close",
      successMessage: "Your order has been placed successfully!",
    },
    deleteModal: {
      title: "Delete this item?",
      message: "This item will be permanently deleted and this action cannot be undone.",
      cancelButton: "Cancel",
      confirmButton: "Yes, delete it",
      deleting: "Deleting...",
      successMessage: "Item deleted successfully",
      errorMessage: "Failed to delete the item, please try again",
    },
  },
  myInventory: {
    title: "My Inventory",
    subtitle: "Manage your store products, stock levels, and available inventory",
    totalItems: "Total Items:",
    searchPlaceholder: "Search by product name...",
    allCategories: "All Categories",
    searchBtn: "Search",
    resetBtn: "Reset",
    noDataTitle: "No inventory found",
    noDataSub: "Try adjusting your search criteria or add new items to your stock.",
    statusAvailable: "Available",
    statusDepleted: "Depleted",
    totalQty: "Total Qty:",
    pricePerUnit: "Price/Unit:",
    viewDetails: "View Details",
    page: "Page",
    of: "of",
    prevBtn: "Previous",
    nextBtn: "Next",
  },
  editInventory: {
    title: "Edit Inventory Item",
    subtitle: "Update existing product details, pricing, and stock quantity.",
    basicInfoLabel: "Basic Information",
    productNameLabel: "Product Name",
    productNamePlaceholder: "e.g., Premium Miniket Rice",
    categoryLabel: "Category",
    selectCategory: "Select Category",
    unitLabel: "Unit",
    selectUnit: "Select Unit",
    quantityPricingLabel: "Quantity & Pricing",
    totalQuantityLabel: "Total Quantity",
    allocatedQuantityLabel: "Allocated Quantity",
    pricePerUnitLabel: "Price Per Unit",
    detailsLabel: "Additional Details & Image",
    descriptionLabel: "Description",
    descriptionPlaceholder: "Enter short product description...",
    imageLabel: "Product Image",
    uploadClick: "Click to change image",
    uploadDrag: "or drag and drop",
    uploadHint: "PNG, JPG or WEBP (Leave blank to keep existing image)",
    submitButton: "Update Inventory",
    submitting: "Updating Inventory...",
    loadingText: "Loading inventory details...",
    units: {
      kg: "Kg",
      mon: "Mon",
      ton: "Ton",
      piece: "Piece",
    },
    errors: {
      productNameRequired: "Product name cannot be empty",
      categoryRequired: "Invalid category",
      unitRequired: "Invalid unit value",
      totalQuantityRequired: "Total quantity is required",
      totalQuantityMin: "Total quantity must be at least 1",
      allocatedQuantityRequired: "Allocated quantity is required",
      allocatedQuantityMin: "Allocated quantity must be at least 0",
      allocatedGreaterThanTotal: "Allocated quantity cannot be greater than total quantity",
      pricePerUnitRequired: "Price per unit is required",
      pricePerUnitMin: "Price per unit must be at least 1",
      descriptionMax: "Description must be maximum 300 characters",
      onlyImageAllowed: "Only image files are allowed",
    },
  },
  allInventory: {
    title: "Inventory List",
    subtitle: "Overview of all stocked agricultural products",
    searchPlaceholder: "Search by product name...",
    allCategories: "All Categories",
    noItemsFound: "No inventory items found",
    price: "Price",
    quantity: "Quantity",
    status: "Status",
    available: "Available",
    depleted: "Depleted",
    viewDetails: "View Details",
    page: "Page",
    of: "of",
    previous: "Previous",
    next: "Next",
  },
  aratdarReceiveOrders: {
    title: "Received Orders",
    orderId: "Order ID",
    product: "Product",
    quantity: "Quantity",
    totalAmount: "Total Amount",
    status: "Status",
    date: "Date",
    noOrders: "No orders found",
    loading: "Loading...",
    previous: "Previous",
    next: "Next",
    page: "Page",
    of: "of",
  },
  aratdarReceiveOrderDetails: {
    title: "Order Details",
    backToList: "Back to List",
    orderDetails: "Order Details",
    buyerDetails: "Buyer Information",
    productDetails: "Product Details",
    orderId: "Order ID",
    buyerName: "Buyer Name",
    phone: "Phone Number",
    email: "Email",
    district: "District",
    productName: "Product Name",
    category: "Category",
    quantity: "Quantity",
    totalAmount: "Total Amount",
    currentStatus: "Current Status",
    changeStatus: "Change Status",
    selectStatus: "Select Status",
    updateStatus: "Update Status",
    updating: "Updating...",
    createdAt: "Order Date",
    cancelReason: "Cancel Reason",
    statusUpdatedSuccess: "Status updated successfully",
    invalidTransition: "Invalid status transition",
  },
  retailersPlacedOrders: {
    title: "Order History",
    subtitle: "View and track all orders you have placed",
    orderId: "Order ID",
    product: "Product",
    quantity: "Quantity",
    totalAmount: "Total Amount",
    status: "Status",
    date: "Date",
    action: "Action",
    viewDetails: "View Details",
    noOrders: "No orders found",
    noOrdersDesc: "You haven't placed any orders yet or no orders match the selected filter.",
    page: "Page",
    of: "of",
    previous: "Previous",
    next: "Next",
    allStatus: "All Status",
  },
  retailerOrderDetails: {
    backToList: "Back to Order List",
    orderTitle: "Order Details",
    totalAmount: "Total Amount",
    sellerDetails: "Seller Information",
    sellerName: "Seller Name",
    phone: "Phone Number",
    email: "Email",
    productDetails: "Product Details",
    quantity: "Quantity",
    cancelOrder: "Cancel Order",
    cancelReasonTitle: "Reason for Cancellation",
    cancelReasonPlaceholder: "Please enter the reason for cancellation...",
    cancelReasonRequired: "Cancellation reason is required",
    confirmCancel: "Confirm Cancellation",
    close: "Close",
    cancelledSuccess: "Order cancelled successfully",
    cancelReasonLabel: "Cancellation Reason",
    cancelling: "Cancelling...",
    orderNotCancelable: "Only PENDING orders can be cancelled",
    report: "Report"
  },
  adminDashboard: {
    title: "Admin Dashboard",
    subtitle: "An overview of users, orders, and platform activity",
    stats: {
      totalUsers: "Total Users",
      farmers: "Farmers",
      aratdars: "Aratdars",
      retailers: "Retailers",
      products: "Products",
      inventories: "Inventories",
      activeAuctions: "Active Auctions",
      orders: "Orders",
      pendingReports: "Pending Reports",
      pendingMemberRequest: "Pending Member Requests",
    },
    charts: {
      userGrowth: "User Growth",
      orderGrowth: "Order Growth",
      orderStatus: "Order Status",
      userRoles: "User Roles",
    },
    recentOrders: {
      title: "Recent Orders",
      viewAll: "View all",
      seller: "Seller",
      buyer: "Buyer",
      status: "Status",
      date: "Date",
      empty: "No recent orders yet",
    },
    roles: {
      farmer: "Farmer",
      aratdar: "Aratdar",
      retailer: "Retailer",
      admin: "Admin",
    },
  },
  farmerDashboard: {
    title: "Farmer dashboard",
    subtitle: "Your crops, orders and auctions at a glance",
    currency: "৳",
    noData: "No data yet",
    loadFailed: "Could not load your dashboard. Please refresh the page.",
    waitingAlert: "auction(s) waiting for you to choose a winner",
    waitingAlertAction: "Review",
    stats: {
      totalCrops: "Total crops",
      availableCrops: "Available crops",
      totalOrders: "Total orders",
      pendingOrders: "Pending orders",
      deliveredOrders: "Delivered orders",
      totalSales: "Total sales",
      totalAuctions: "Total auctions",
      activeAuctions: "Active auctions",
      waitingAuctionSelection: "Waiting for selection",
      completedAuctions: "Completed auctions",
    },
    charts: {
      salesTrend: "Monthly sales",
      orderTrend: "Monthly orders",
      cropStatus: "Crop status",
      orderStatus: "Order status",
      sales: "Sales",
      orders: "Orders",
    },
    cropStats: {
      available: "Available",
      sold: "Sold",
      expired: "Expired",
    },
    orderStats: {
      pending: "Pending",
      delivered: "Delivered",
      cancelled: "Cancelled",
    },
    orderStatus: {
      PENDING: "Pending",
      CONFIRMED: "Confirmed",
      PROCESSING: "Processing",
      SHIPPED: "Shipped",
      DELIVERED: "Delivered",
      CANCELLED: "Cancelled",
    },
    units: {
      kg: "kg",
      mon: "mon",
      ton: "ton",
      piece: "piece",
    },
    recentOrders: {
      title: "Recent orders",
      viewAll: "View all",
      empty: "No orders yet. New orders will show up here.",
      product: "Product",
      buyer: "Buyer",
      quantity: "Quantity",
      amount: "Amount",
      status: "Status",
      date: "Date",
    },
    auctions: {
      title: "Live auctions",
      viewAll: "View all",
      empty: "No active auctions right now.",
      startPrice: "Start price",
      highestBid: "Highest bid",
      noBids: "No bids yet",
      timeLeft: "Time left",
      ended: "Ended",
      hourShort: "h",
      minShort: "m",
    },
  },
  aratdarDashboard: {
    title: "Aratdar dashboard",
    subtitle: "Your stock, purchases, sales and orders at a glance",
    currency: "৳",
    noData: "No data yet",
    loadFailed: "Could not load your dashboard. Please refresh the page.",
    lowStockAlert: "item(s) are running low on stock",
    lowStockAlertAction: "Review",
    stats: {
      totalSales: "Total sales",
      totalPurchase: "Total purchase",
      totalInventoryItems: "Inventory items",
      availableInventoryItems: "In stock",
      depletedInventoryItems: "Out of stock",
      lowStockItems: "Low stock",
      totalOrders: "Total orders",
      pendingOrders: "Pending orders",
      deliveredOrders: "Delivered orders",
      cancelledOrders: "Cancelled orders",
    },
    charts: {
      purchaseVsSales: "Monthly purchase vs sales",
      orderTrend: "Monthly orders",
      orderStatus: "Order status",
      stockUsage: "Stock usage",
      purchase: "Purchase",
      sales: "Sales",
      purchaseOrders: "Purchase orders",
      salesOrders: "Sales orders",
    },
    orderStats: {
      pending: "Pending",
      delivered: "Delivered",
      cancelled: "Cancelled",
      inProgress: "In progress",
    },
    stock: {
      allocated: "Allocated",
      remaining: "Remaining",
    },
    orderStatus: {
      PENDING: "Pending",
      CONFIRMED: "Confirmed",
      PROCESSING: "Processing",
      SHIPPED: "Shipped",
      DELIVERED: "Delivered",
      CANCELLED: "Cancelled",
    },
    units: {
      kg: "kg",
      mon: "mon",
      ton: "ton",
      piece: "piece",
    },
    inventory: {
      title: "My inventory",
      viewAll: "View all",
      empty: "No inventory items yet.",
      allocated: "Allocated",
      remaining: "Remaining",
      status: {
        available: "In stock",
        depleted: "Out of stock",
      },
    },
    recentPurchases: {
      title: "Recent purchases",
      viewAll: "View all",
      empty: "No purchases yet.",
    },
    recentSales: {
      title: "Recent sales",
      viewAll: "View all",
      empty: "No sales yet.",
    },
  },
  retailerDashboard: {
    title: "Retailer dashboard",
    subtitle: "Your purchases and order progress at a glance",
    currency: "৳",
    noData: "No data yet",
    loadFailed: "Could not load your dashboard. Please refresh the page.",
    stats: {
      totalPurchase: "Total purchase",
      totalOrders: "Total orders",
      inProgressOrders: "Orders in progress",
      deliveredOrders: "Delivered orders",
      cancelledOrders: "Cancelled orders",
    },
    charts: {
      purchaseTrend: "Monthly purchase",
      orderTrend: "Monthly orders",
      orderStatus: "Order status",
      purchase: "Purchase",
      orders: "Orders",
    },
    orderStatus: {
      PENDING: "Pending",
      CONFIRMED: "Confirmed",
      PROCESSING: "Processing",
      SHIPPED: "Shipped",
      DELIVERED: "Delivered",
      CANCELLED: "Cancelled",
    },
    units: {
      kg: "kg",
      mon: "mon",
      ton: "ton",
      piece: "piece",
    },
    recentOrders: {
      title: "Recent orders",
      viewAll: "View all",
      empty: "No orders yet. Your orders will show up here.",
    },
  },
  biddingProduct: {
    title: "My Bidding Products",
    subtitle: "Manage and monitor all your bidded product listings",
    searchPlaceholder: "Search by product name...",
    searchBtn: "Search",
    allCategories: "All Categories",
    reset: "Reset",
    noProducts: "No Bidding Products Found",
    noProductsDesc: "We couldn't find any bidding products matching your criteria.",
    myBid: "My Bid:",
    highestBid: "Highest Bid:",
    ends: "Ends:",
    active: "Active",
    expired: "Expired",
    sold: "Sold",
    previous: "Previous",
    next: "Next",
    page: "Page",
    of: "of",
    leading: "Leading",
    outbid: "Outbid",
    timeLeft: "Time left",
    ended: "Ended",
    daysShort: "d",
    hoursShort: "h",
    minutesShort: "m",
  },
};

export default en;
