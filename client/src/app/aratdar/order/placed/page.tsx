...
import { getAratdarPlacedOrders } from '@/store/slice/orderSlice'
...
const Page = () => {
    ...
    const { t } = useLanguage()
    const { orderLoading, aratdarPlaceOrders } = useSelector((state: RootState) => state.order)
    ...
    useEffect(() => {
        if (pageQuery && aratdarPlaceOrders?.pagination?.currentPage !== currentPage) {
            dispatch(getAratdarPlacedOrders({ page: currentPage }))
        }
    }, [dispatch, pageQuery, currentPage, aratdarPlaceOrders?.pagination?.currentPage])
    ...
    // Helper for Status Stamp styling — green/white theme, icon carries the meaning
    ...
    return (
        ...
        >
            <div className="max-w-6xl mx-auto space-y-8">
                    ...
                    <div className="relative z-10 pl-8 pr-6 py-8 sm:pl-12 sm:pr-10 sm:py-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
                        <div className="space-y-3">
                            <div className="inline-flex items-center gap-2 text-white/80 text-xs font-bold tracking-[0.2em] uppercase">
                                <Leaf size={14} />
                                <span>AgriLink Mandi Ledger</span>
                            </div>
                            <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-white">
                                {t.aratdarOrders.title}
                            </h1>
                            <p className="text-sm sm:text-base text-white/70 max-w-xl leading-relaxed">
                                {t.aratdarOrders.subtitle}
                            </p>
                        </div>

                        {/* Folio total-orders tag */}
                        {aratdarPlaceOrders?.pagination?.totalOrders !== undefined && (
                            <div className="flex items-center gap-4 bg-white/10 border border-white/20 p-4 rounded-xl shrink-0 backdrop-blur-sm">
                                ...
                                <div>
                                    ...
                                    <p className="text-xl font-bold text-white tracking-tight">
                                        {aratdarPlaceOrders.pagination.totalOrders}
                                        <span className="text-sm font-normal text-white/60"> Placed</span>
                                    </p>
                                </div>
                            </div>
                        )}
                ...
                {orderLoading ? (
                ...
                ) : !aratdarPlaceOrders?.orders || aratdarPlaceOrders.orders.length === 0 ? (

                    /* Empty State — a blank ledger page */
                    ...
                    <div className="bg-white rounded-2xl border border-[#0F3D2E]/12 shadow-sm overflow-hidden">
                        {aratdarPlaceOrders.orders.map((order, idx) => (
                            <Link
                                key={order._id}
                                href={`/aratdar/order/placed/${order._id}`}
                ...
                {/* Pagination — Folio turner */}
                {!orderLoading && aratdarPlaceOrders?.orders && aratdarPlaceOrders.orders.length > 0 &&
                    aratdarPlaceOrders.pagination && aratdarPlaceOrders.pagination.totalPages > 1 && (
                        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white px-6 py-4 rounded-2xl border border-[#0F3D2E]/12 shadow-sm">
                            <p className="text-xs sm:text-sm text-[#4B5D53] font-medium">
                                {t.aratdarOrders.page} <span className="font-bold text-[#142B20]">{aratdarPlaceOrders.pagination.currentPage}</span> {t.aratdarOrders.of}{' '}
                                <span className="font-bold text-[#142B20]">{aratdarPlaceOrders.pagination.totalPages}</span>
                            </p>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                >
                                    <ChevronLeft size={16} />
                                    {t.aratdarOrders.previous}
                                </button>

                                <button
                                    onClick={() => handlePageChange(currentPage + 1)}
                                    disabled={currentPage >= aratdarPlaceOrders.pagination.totalPages}
                                    className="flex items-center gap-1.5 px-4 py-2 text-xs sm:text-sm font-semibold text-[#142B20] bg-[#F3F9F5] hover:bg-[#E6F4EA] border border-[#0F3D2E]/15 rounded-xl disabled:opacity-40 disabled:hover:bg-[#F3F9F5] disabled:cursor-not-allowed transition-all"
                                >
                                    {t.aratdarOrders.next}
