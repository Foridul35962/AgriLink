...
import { getAratdarPlacedOrderDetails } from '@/store/slice/orderSlice'
...
const Page = () => {
  ...
  const { t } = useLanguage()
  const { orderLoading, aratdarPlaceOrderDetails } = useSelector((state: RootState) => state.order)

  useEffect(() => {
    const fetchDetails = async () => {
      await dispatch(getAratdarPlacedOrderDetails({ orderId: orderId as string })).unwrap()
    }
    if (orderId && orderId !== aratdarPlaceOrderDetails?._id) {
      fetchDetails()
    }
  }, [dispatch, orderId, aratdarPlaceOrderDetails?._id])

  // Same stamp treatment as the list page — icon carries meaning, palette stays green/white
  ...
  // A single label/value ledger row, separated by a dashed rule
  const LedgerRow = ({
    icon: Icon,
    label,
    value,
        ...
        {orderLoading ? (
          ...
          <div className="bg-white rounded-2xl border-2 border-dashed border-[#0F3D2E]/20 p-12 sm:p-16 text-center flex flex-col items-center justify-center">
            ...
            <h3 className="text-xl font-bold text-[#142B20] mb-2">
              {t.aratdarOrderDetails.notFound}
            </h3>
            <p className="text-[#4B5D53] text-sm max-w-md leading-relaxed">
              {t.aratdarOrderDetails.notFoundSub}
            </p>
          </div>
        ) : (
          <>
              ...
              <div className="relative z-10 pl-8 pr-6 py-7 sm:pl-12 sm:pr-10 sm:py-8 flex flex-col sm:flex-row sm:items-center justify-between gap-5">
                <div className="space-y-2.5">
                  <div className="inline-flex items-center gap-2 text-white/80 text-xs font-bold tracking-[0.2em] uppercase">
                    <BookOpen size={14} />
                    <span>{t.aratdarOrderDetails.title}</span>
                  </div>
                  <p className="text-lg sm:text-xl font-mono font-bold text-white tracking-wide">
                    #{aratdarPlaceOrderDetails._id.slice(-8).toUpperCase()}
                  </p>
                  <span className="inline-flex items-center gap-1.5 text-xs text-white/70">
                    <Calendar size={12} />
                    {new Date(aratdarPlaceOrderDetails.createdAt).toLocaleDateString(undefined, {
                      year: 'numeric',
                      month: 'long',
                      day: 'numeric'
                    })}
                ...
                <div>{getStatusBadge(aratdarPlaceOrderDetails.status)}</div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-[#0F3D2E]/12 shadow-sm p-5 sm:p-6 flex flex-col sm:flex-row items-start sm:items-center gap-5">
              ...
              <div className="space-y-2 min-w-0">
                <h1 className="text-xl sm:text-2xl font-extrabold text-[#142B20] truncate">
                  {aratdarPlaceOrderDetails.productId?.name || 'N/A'}
                </h1>
                <div className="flex flex-wrap items-center gap-2">
                  <span className="inline-flex items-center gap-1.5 bg-[#F3F9F5] border border-[#0F3D2E]/15 px-2.5 py-1 rounded-md text-xs font-medium text-[#4B5D53]">
                    ...
                    {aratdarPlaceOrderDetails.productId?.category}
                  </span>
                  <span className="inline-flex items-center gap-1.5 bg-[#F3F9F5] border border-[#0F3D2E]/15 px-2.5 py-1 rounded-md text-xs font-medium text-[#4B5D53]">
                    <MapPin size={12} className="text-[#4B7C5E]" />
                    {aratdarPlaceOrderDetails.productId?.district}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-[#0F3D2E]/12 shadow-sm p-5 sm:p-6">
              <h2 className="text-xs font-bold uppercase tracking-widest text-[#4B7C5E] mb-1">
                {t.aratdarOrderDetails.orderInfo}
              </h2>
              <LedgerRow icon={Layers} label={t.aratdarOrderDetails.quantity} value={`${aratdarPlaceOrderDetails.quantity} ${aratdarPlaceOrderDetails.unit}`} />
              <LedgerRow
                icon={BookOpen}
                label={t.aratdarOrderDetails.totalAmount}
                value={
                  <span className="border-b-2 border-[#1F7A4B]/40">
                    {t.aratdarOrderDetails.currency}{aratdarPlaceOrderDetails.totalAmount.toLocaleString()}
                  </span>
                }
              />
            </div>
            ...
            <div className="bg-white rounded-2xl border border-[#0F3D2E]/12 shadow-sm p-5 sm:p-6">
              <h2 className="text-xs font-bold uppercase tracking-widest text-[#4B7C5E] mb-1">
                {t.aratdarOrderDetails.sellerInfo}
              </h2>
              <LedgerRow icon={User} label={t.aratdarOrderDetails.sellerName} value={aratdarPlaceOrderDetails.sellerId?.name || t.aratdarOrderDetails.notProvided} />
              <LedgerRow icon={Phone} label={t.aratdarOrderDetails.phoneNumber} value={aratdarPlaceOrderDetails.sellerId?.phoneNumber || t.aratdarOrderDetails.notProvided} />
              <LedgerRow icon={Mail} label={t.aratdarOrderDetails.email} value={aratdarPlaceOrderDetails.sellerId?.email || t.aratdarOrderDetails.notProvided} />
            </div>
          </>
        )}
      </div>
